...
import { sendNewCaseAdminNotification } from '$lib/server/case-notify';
...
export const POST: RequestHandler = async ({ request, locals: { supabase, safeGetSession } }) => {
	...
	try {
		await sendNewCaseAdminNotification(supabase, user.id, caseId);
	} catch (err) {
		const message = err instanceof Error ? err.message : 'No se pudo enviar la notificación.';
		console.error('[api/notify/case-created]', message);
		error(502, message);
	}
