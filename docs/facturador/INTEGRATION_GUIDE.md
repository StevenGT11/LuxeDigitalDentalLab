...
Package: **`@happy-prod/facturador`** (library + optional HTTP API in the same repo).
...
npx @happy-prod/facturador install-mcp
```
...
| CORS from browser | Call from SvelteKit server only; set `CORS_ORIGINS` only if you must allow a browser origin |
